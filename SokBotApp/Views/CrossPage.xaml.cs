...
{
    /// <summary>
    /// Логика взаимодействия для CrossPage.xaml
    /// </summary>
    public partial class CrossPage : Page
    {
        public object currentEvent;

        public CrossPage(object obj)
        {
            InitializeComponent();

            if (obj is Cross cross)
            {
                currentEvent = obj;
                cross.DownloadEventFile(cross.EventFolderName);
                cross.GetMembers(cross.EventFileName);
                DataContext = new CrossViewModel(cross.Members);
            }
